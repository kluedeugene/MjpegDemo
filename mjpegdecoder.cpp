...
int MjpegDecoder::initial()
{
         //avcodec_register_all();//The new version should not need this sentence
...
int MjpegDecoder::decode(uint8_t *pDataIn, int nInSize, cv::Mat& res)
{
//    av_init_packet(&packet);
    packet.size = nInSize;
    packet.data = pDataIn;
    ...
    if (packet.size > 0)
    {
        ...
        if (ret < 0)
        {
            ...
            return -2;
                 ...
                 if (got_picture==0)//When using avcodec_decode_video2, here is if (got_picture)
        {
...
int MjpegDecoder::avframe_to_cvmat(AVFrame *frame,cv::Mat& res)
{
    ...
    memcpy( res.data + width*height*5/4, frame->data[2], width*height/4 );
         ...
         //cv::imshow( "yuv_show", res );//yuv format
         cv::cvtColor( res, res, cv::COLOR_YUV2BGR_I420 );//bgr format
    //cv::imshow( "bgr_show", bgr );
    return 0;
 
}
AVFrame * MjpegDecoder::cvmat2avframe(cv::Mat mat) {
